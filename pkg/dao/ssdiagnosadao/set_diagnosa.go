package ssdiagnosadao

import (
	"errors"

	"git.solusiteknologi.co.id/goleaf/goleafcore"
	"git.solusiteknologi.co.id/goleaf/goleafcore/glconstant"
	"git.solusiteknologi.co.id/goleaf/goleafcore/gldata"
	"git.solusiteknologi.co.id/goleaf/goleafcore/gldb"
	"git.solusiteknologi.co.id/goleaf/goleafcore/glutil"
	"git.solusiteknologi.co.id/sts-satusehat/ssbackend/pkg/tables"
	"github.com/sirupsen/logrus"
)

type InputSet struct {
	Tx    gldb.Tx
	Audit gldata.AuditData

	DiagnosaCode    string `json:"diagnosaCode" validate:"required"`
	DiagnosaName    string `json:"diagnosaName" validate:"required"`
	DiagnosaVersion string `json:"diagnosaVersion"`
}

func Set(input InputSet) (*tables.Diagnosa, error) {
	logrus.Debug(" -- START Set Diagnosa -> ")
	result := tables.Diagnosa{}
	if err := glutil.FetchDefAndValidate(&input); err != nil {
		return nil, err
	}

	err := gldb.SelectOneQTx(input.Tx, *gldb.NewQBuilder().
		Add(" INSERT INTO ", tables.SS_DIAGNOSA).
		AddValue("tenant_id", input.Audit.TenantLoginId).
		AddValue("diagnosa_code", input.DiagnosaCode).
		AddValue("diagnosa_name", input.DiagnosaName).
		AddValue("diagnosa_version", input.DiagnosaVersion).
		AddValue("active", glconstant.YES).
		AddValue("active_datetime", input.Audit.Datetime()).
		AddValue("non_active_datetime", glconstant.EMPTY_VALUE).
		AddValueInsertAudit(input.Audit.Datetime(), input.Audit.UserLoginId).
		Add(` ON CONFLICT (tenant_id, diagnosa_code) DO UPDATE 
            SET diagnosa_name = EXCLUDED.diagnosa_name,
                diagnosa_version = EXCLUDED.diagnosa_version,
                update_datetime = EXCLUDED.update_datetime,
                update_user_id = EXCLUDED.update_user_id
            RETURNING `, result.Columns()), &result)

	if err != nil {
		return nil, errors.New("failed insert diagnosa if not exists : " + err.Error())
	}
	logrus.Debug("Diagnosa Data : ", goleafcore.NewOrEmpty(result).PrettyString())
	logrus.Debug(" -- END Set Diagnosa -> ")
	return &result, nil
}
